...
public class PlayerControlView extends FrameLayout {
  ...
  public static Boolean doubleTapStatus = false;
  public static long doubleTapStopPos = 0;
  ...
  private final View fastForwardButton = null;
  private final View rewindButton = null;
  private final ImageView repeatToggleButton = null;
  private final View shuffleButton = null;
  ...
  private final View rstartButton = null;
  private final View rendButton = null;
  private final View rclearButton = null;
  ...
  private long repeatStartPos;
  private long repeatEndPos;
  private boolean repeatStatus;

  public boolean controllerViewStatus;
  ...
  private final Runnable repeatProgressAction =
          new Runnable() {
            @Override
            public void run() {
              repeatProgress();
  ...
  public PlayerControlView(
    ...
    positionView = findViewById(R.id.current_time_view);
    timeBar = findViewById(R.id.progress_view);
    if (timeBar != null) {
      timeBar.addListener(componentListener);
    }

//    rstartButton = findViewById(R.id.exo_repeat_start);
    if (rstartButton != null) {
      rstartButton.setOnClickListener(componentListener);
    }
//    rendButton = findViewById(R.id.exo_repeat_end);
    if (rendButton != null) {
      rendButton.setOnClickListener(componentListener);
      rendButton.setVisibility(View.GONE);
    }
//    rclearButton = findViewById(R.id.exo_repeat_clear);
    if (rclearButton != null) {
      rclearButton.setOnClickListener(componentListener);
      rclearButton.setVisibility(View.GONE);
    }
...
//    repeatAllButtonDrawable = resources.getDrawable(R.drawable.exo_controls_repeat_all);
    ...
    repeatStatus = false;
    controllerViewStatus = true;
    postDelayed(repeatProgressAction, 200);
  }
...
//    Log.d("mcn","updateProgress()");
    if(isPlaying() && repeatStatus && repeatEndPos < player.getContentPosition()){
      player.seekTo(repeatStartPos);
    }
    else if(isPlaying() && repeatStatus && repeatStartPos > player.getContentPosition()){
      player.seekTo(repeatStartPos);
    }
    postDelayed(repeatProgressAction, 200);
  ...
  private final class ComponentListener extends Player.DefaultEventListener
    ...
    public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
      updatePlayPauseButton();
      updateProgress();
      Log.d("mcn", "onPlayerStateChanged");
    ...
    public void onShuffleModeEnabledChanged(boolean shuffleModeEnabled) {
      updateShuffleButton();
      updateNavigation();
      Log.d("mcn", "onShuffleModeEnabledChanged");
      ...
      if (player != null) {
        if (nextButton == view) {
          ...
          if (player.getPlaybackState() == Player.STATE_IDLE) {
            //mcn double tap play button modified
            if (playbackPreparer != null && doubleTapStatus) {
              doubleTapStatus = false;
              playbackPreparer.preparePlayback();
              controlDispatcher.dispatchSeekTo(player, player.getCurrentWindowIndex(),doubleTapStopPos);
            }
            else if (playbackPreparer != null) {
                playbackPreparer.preparePlayback();
            }
        ...
        else if (rstartButton == view) {
          rstartButton.setVisibility(View.GONE);
          rendButton.setVisibility(View.VISIBLE);
          setRepeatStartPos();
        }
        else if (rendButton == view) {
          rendButton.setVisibility(View.GONE);
          rclearButton.setVisibility(View.VISIBLE);
          setRepeatEndPos();
        }
        else if (rclearButton == view) {
          rclearButton.setVisibility(View.GONE);
          rstartButton.setVisibility(View.VISIBLE);
          clearTimeBarRepeat();
        }
      }
      hideAfterTimeout();
  ...
  public void setRepeatStartPos(){
    if(timeBar == null ) return;
    DefaultTimeBar tBar = (DefaultTimeBar)timeBar;
    tBar.setRepeatStartPoint();
    repeatStartPos = player.getCurrentPosition();
  }
  public void setRepeatEndPos(){
    if(timeBar == null || player.getCurrentPosition() < repeatStartPos) return;
    DefaultTimeBar tBar = (DefaultTimeBar)timeBar;
    tBar.setRepeatEndPoint();
    repeatEndPos = player.getCurrentPosition();
    repeatStatus = true;
  }
  public void clearTimeBarRepeat(){
    DefaultTimeBar tBar = (DefaultTimeBar)timeBar;
    tBar.clearRepeat();
    repeatStatus = false;
  }
